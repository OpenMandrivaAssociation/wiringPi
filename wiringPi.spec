...
%else
Source0:	https://github.com/WiringPi/WiringPi/archive/refs/tags/%(echo %{version} |sed -e 's,_,-,g').tar.gz
%endif
...
%prep
%autosetup -p1 -n WiringPi-%{?git:master}%{!?git:%(echo %{version}|sed -e 's,_,-,g')}
...
%endif
sed -i -e 's,/sbin,/bin,g' wiringPiD/Makefile

%build
HDRFLAGS="-I$(pwd)/wiringPi -I$(pwd)/devLib"
LIBFLAGS="-L$(pwd)/"

ln -s wiringPi/libwiringPi.so.%(echo %{version}|sed -e 's,_.*,,') libwiringPi.so
ln -s devLib/libwiringPiDev.so.%(echo %{version} |sed -e 's,_.*,,') libwiringPiDev.so
